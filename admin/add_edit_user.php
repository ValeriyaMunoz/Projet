...
if (!empty($_POST)) {
    ...
    $titre = $_POST['is_admin'] ?? '';
    $is_admin = filter_input(INPUT_POST, 'is_admin', FILTER_SANITIZE_NUMBER_INT);

    // Création de l'objet BDD
    $db = connect();

    // Si un membre a un ID, il est déjà enregistré en BDD, donc on le met à jour, sinon on le crée.
    if (empty($_POST['id'])) {
        // L'abonnement n'est pas dans la BDD, on le crée
        try {
            // Préparation de la requête d'insertion
            $createuseris_admin = $db->prepare('INSERT INTO membre (id, is_admin) VALUES (:id, :is_admin)');
